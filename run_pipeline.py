...
请分两步思考（可分段写）：
1）哪一张图最清楚地概括**论文要解决的任务/问题**？对应哪个 id？
2）哪一张图最像**pipeline、算法框架结构流程图**？对应哪个 id？
若某类在图注中难以判断，说明依据并给出你仍认为最贴近的 id。二者不可重复。

最后一行请用一句话总结你准备推荐的两个 id（仍用 x# 表示）。"""

PROMPT_STEP07B_SELECT_STEP2 = """下面是你在上一轮对「任务相关图」与「pipeline图」的分析草稿：
{step1_output}
...
- 第 1 个元素：最贴「论文任务」的图 id；
- 第 2 个元素：最贴「pipeline图」的图 id。
